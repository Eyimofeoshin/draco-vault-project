...
var bls12_381 = bls({
  ...
  G2: {
    ...
    fromBytes: (bytes) => {
      ...
      const L = Fp.BYTES;
      const slc = (b, from25, to2) => bytesToNumberBE(b.slice(from25, to2));
...
function fromCreate(options) {
  ...
  let nonce3 = fromNumber(options.nonce);
  if (nonce3[0] === 0)
    nonce3 = new Uint8Array([]);
  return from4(`0x${keccak256(fromBytes2([from25, nonce3], { as: "Hex" })).slice(26)}`);
}
function fromCreate2(options) {
  ...
  return from4(slice2(keccak256(concat2(fromHex("0xff"), from25, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function split(name, nf, ef) {
  ...
  return name.split(STOP_CH).map((label) => {
    ...
    try {
      ...
      let emoji = info.emoji = token_count > 1 || tokens2[0].is_emoji;
...
function join(separator = "") {
  ...
  return {
    encode: (from25) => {
      astrArr("join.decode", from25);
      return from25.join(separator);
    },
    decode: (to2) => {
...
function normalize2(fn) {
  afn(fn);
  return { encode: (from25) => from25, decode: (to2) => fn(to2) };
}
function convertRadix(data, from25, to2) {
  if (from25 < 2)
    throw new Error(`convertRadix: invalid from=${from25}, base cannot be less than 2`);
  if (to2 < 2)
  ...
  while (true) {
    ...
    for (let i = pos; i < dlen; i++) {
      ...
      const fromCarry = from25 * carry;
      const digitBase = fromCarry + digit;
      if (!Number.isSafeInteger(digitBase) || fromCarry / from25 !== carry || digitBase - digit !== fromCarry) {
...
var radix2carry = (from25, to2) => from25 + (to2 - gcd(from25, to2));
var powers = (() => {
  let res = [];
  for (let i = 0; i < 40; i++)
    res.push(2 ** i);
...
function convertRadix2(data, from25, to2, padding2) {
  aArr(data);
  if (from25 <= 0 || from25 > 32)
    throw new Error(`convertRadix2: wrong from=${from25}`);
  if (to2 <= 0 || to2 > 32)
    throw new Error(`convertRadix2: wrong to=${to2}`);
  if (radix2carry(from25, to2) > 32) {
    throw new Error(`convertRadix2: carry overflow from=${from25} to=${to2} carryBits=${radix2carry(from25, to2)}`);
  }
  let carry = 0;
  let pos = 0;
  const max = powers[from25];
  const mask = powers[to2] - 1;
  const res = [];
  for (const n of data) {
    anumber2(n);
    if (n >= max)
      throw new Error(`convertRadix2: invalid data word=${n} from=${from25}`);
    carry = carry << from25 | n;
    if (pos + from25 > 32)
      throw new Error(`convertRadix2: carry overflow pos=${pos} from=${from25}`);
    pos += from25;
    for (; pos >= to2; pos -= to2)
  ...
  function wrappedCipher(key, ...args) {
    ...
    if (params.nonceLength !== void 0) {
      ...
      if (params.varSizeNonce)
        ...
        abytes3(nonce3, params.nonceLength);
    }
...
function ctrCounter(xk, nonce3, src, dst) {
  abytes3(nonce3, BLOCK_SIZE2);
  abytes3(src);
...
function ctr32(xk, isLE2, nonce3, src, dst) {
  abytes3(nonce3, BLOCK_SIZE2);
  abytes3(src);
...
var ctr = wrapCipher({ blockSize: 16, nonceLength: 16 }, function aesctr(key, nonce3) {
...
var gcm = wrapCipher({ blockSize: 16, nonceLength: 12, tagLength: 16, varSizeNonce: true }, function aesgcm(key, nonce3, AAD) {
  if (nonce3.length < 8)
  ...
  function deriveKeys() {
    ...
    if (nonce3.length === 12) {
      ...
      setBigUint64(view, 8, BigInt(nonce3.length * 8), false);
      const g = ghash.create(authKey).update(nonce3).update(nonceLen);
...
var gcmsiv = wrapCipher({ blockSize: 16, nonceLength: 12, tagLength: 16, varSizeNonce: true }, function aessiv(key, nonce3, AAD) {
  const tagLength = 16;
...
function createMessage(value) {
  const { chainId, domain, expirationTime, issuedAt = /* @__PURE__ */ new Date(), nonce: nonce3, notBefore, requestId, resources, scheme, uri, version: version2 } = value;
  {
...
function validateMessage(value) {
  const { address, domain, message, nonce: nonce3, scheme, time = /* @__PURE__ */ new Date() } = value;
  if (domain && message.domain !== domain)
    return false;
  if (nonce3 && message.nonce !== nonce3)
    return false;
...
function deserialize2(serialized) {
  ...
  const [chainId, nonce3, maxPriorityFeePerGas, maxFeePerGas, gas, to2, value, data, accessList, yParity, r, s] = transactionArray;
  ...
  if (validate2(nonce3))
    transaction.nonce = nonce3 === "0x" ? 0n : BigInt(nonce3);
...
function serialize(envelope, options = {}) {
  const { chainId, gas, nonce: nonce3, to: to2, value, maxFeePerGas, maxPriorityFeePerGas, accessList, data, input } = envelope;
  assert3(envelope);
  const accessTupleList = toTupleList2(accessList);
  const signature = extract(options.signature || envelope);
  const serialized = [
    fromNumber2(chainId),
    nonce3 ? fromNumber2(nonce3) : "0x",
...
function deserialize3(serialized) {
  const transactionArray = toHex3(slice2(serialized, 1));
  const [chainId, nonce3, gasPrice, gas, to2, value, data, accessList, yParity, r, s] = transactionArray;
...
function serialize2(envelope, options = {}) {
  const { chainId, gas, data, input, nonce: nonce3, to: to2, value, accessList, gasPrice } = envelope;
  assert4(envelope);
  const accessTupleList = toTupleList2(accessList);
  const signature = extract(options.signature || envelope);
  const serialized = [
...
function deserialize4(serialized) {
  ...
  const [chainId, nonce3, maxPriorityFeePerGas, maxFeePerGas, gas, to2, value, data, accessList, maxFeePerBlobGas, blobVersionedHashes, yParity, r, s] = transactionArray;
...
function serialize3(envelope, options = {}) {
  const { blobVersionedHashes, chainId, gas, nonce: nonce3, to: to2, value, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, accessList, data } = envelope;
  assert5(envelope);
  const accessTupleList = toTupleList2(accessList);
  const signature = extract(options.signature || envelope);
  const serialized = [
...
function deserialize5(serialized) {
  const transactionArray = toHex3(slice2(serialized, 1));
  const [chainId, nonce3, maxPriorityFeePerGas, maxFeePerGas, gas, to2, value, data, accessList, authorizationList, yParity, r, s] = transactionArray;
...
function serialize4(envelope, options = {}) {
  const { authorizationList, chainId, gas, nonce: nonce3, to: to2, value, maxFeePerGas, maxPriorityFeePerGas, accessList, data, input } = envelope;
  assert6(envelope);
  const accessTupleList = toTupleList2(accessList);
...
function deserialize6(serialized) {
  ...
  const [nonce3, gasPrice, gas, to2, value, data, chainIdOrV_, r, s] = tuple;
...
function serialize5(envelope, options = {}) {
  const { chainId = 0, gas, data, input, nonce: nonce3, to: to2, value, gasPrice } = envelope;
  assert7(envelope);
  let serialized = [
    nonce3 ? fromNumber2(nonce3) : "0x",
    gasPrice ? fromNumber2(gasPrice) : "0x",
...
function assert8(value) {
  ...
  const validateData = (struct, data) => {
    for (const param of struct) {
      ...
      if (type6 === "address" && typeof value2 === "string" && !validate3(value2))
...
(function(approve3) {
  ...
  approve3.estimateGas = estimateGas;
...
async function approveSync(client, parameters) {
  ...
  return {
    ...args,
    ...resolved === void 0 ? {} : { decimals: resolved, formatted: formatUnits(args.amount, resolved) },
    receipt
  };
}
...
(function(burnBlocked3) {
  async function inner(action, client, parameters) {
    const { amount, from: from25, token, ...rest } = parameters;
    const call2 = burnBlocked3.call(client, { amount, from: from25, token });
    return await action(client, {
      ...rest,
      ...call2
    });
  ...
  function call(client, args) {
    const { from: from25, amount, token } = args;
    const { address, decimals } = resolveToken(client, { token });
    return defineCall({
      address,
      abi: tip20,
      functionName: "burnBlocked",
      args: [from25, toBaseUnits(amount, decimals)]
    });
  }
...
(function(burn6) {
  async function inner(action, client, parameters) {
    ...
    const call2 = burn6.call(client, { amount, memo, token });
    return await action(client, {
      ...rest,
      ...call2
    });
...
(function(changeTransferPolicy3) {
  async function inner(action, client, parameters) {
    ...
    const call2 = changeTransferPolicy3.call(client, { policyId, token });
    return await action(client, {
      ...rest,
      ...call2
    });
...
(function(create8) {
  async function inner(action, client, parameters) {
    ...
    const call2 = create8.call(client, { ...rest, admin: admin.address });
    return await action(client, {
      ...parameters,
      account,
      chain: chain2,
...
async function getMetadata(client, parameters) {
  ...
  const overrides2 = {
    ...(declared == null ? void 0 : declared.decimals) != null ? { decimals: declared.decimals } : {},
    ...(declared == null ? void 0 : declared.name) != null ? { name: declared.name } : {},
    ...(declared == null ? void 0 : declared.symbol) != null ? { symbol: declared.symbol } : {}
  };
  if (TokenId_exports.fromAddress(address) === TokenId_exports.fromAddress(pathUsd))
...
function unwrapMulticallResult(response, ...fallback2) {
  ...
  async function inner(action, client, parameters) {
    return await action(client, {
      ...
      calls: parameters.roles.map((role) => {
        const call2 = grantRoles3.call(client, { ...parameters, role });
        return {
          ...call2,
...
(function(mint5) {
  async function inner(action, client, parameters) {
    const { amount, memo, to: to2, token, ...rest } = parameters;
    const call2 = mint5.call(client, { amount, memo, to: to2, token });
    return await action(client, {
      ...rest,
      ...call2
    });
  ...
  function call(client, args) {
    ...
    const callArgs = memo ? {
      ...
      args: [to2, value, padLeft2(memo, 32)]
    } : {
      functionName: "mint",
      args: [to2, value]
    };
...
(function(pause3) {
  async function inner(action, client, parameters) {
    const { token, ...rest } = parameters;
    const call2 = pause3.call(client, { token });
    return await action(client, {
      ...rest,
      ...call2
    });
...
(function(renounceRoles3) {
  async function inner(action, client, parameters) {
    return await action(client, {
      ...parameters,
      calls: parameters.roles.map((role) => {
        const call2 = renounceRoles3.call(client, { ...parameters, role });
        return {
          ...call2,
          data: encodeFunctionData(call2)
        };
...
(function(revokeRoles3) {
  async function inner(action, client, parameters) {
    ...
    return await action(client, {
      ...rest,
      calls: parameters.roles.map((role) => {
        const call2 = revokeRoles3.call(client, { ...parameters, role });
        return {
          ...call2,
          data: encodeFunctionData(call2)
        };
  ...
  function call(client, args) {
    const { token, from: from25, role } = args;
    const roleHash = TokenRole_exports.serialize(role);
    return defineCall({
      address: resolveToken(client, { token }).address,
      abi: tip20,
      functionName: "revokeRole",
      args: [roleHash, from25]
    });
  }
...
(function(setSupplyCap3) {
  async function inner(action, client, parameters) {
    ...
    const call2 = setSupplyCap3.call(client, { supplyCap, token });
    return await action(client, {
      ...rest,
      ...call2
    });
...
(function(setRoleAdmin3) {
  async function inner(action, client, parameters) {
    ...
    const call2 = setRoleAdmin3.call(client, { adminRole, role, token });
    return await action(client, {
      ...rest,
      ...call2
    });
...
(function(transfer5) {
  async function inner(action, client, parameters) {
    return await action(client, {
      ...parameters,
      ...transfer5.call(client, parameters)
    });
  }
  transfer5.inner = inner;
  function call(client, parameters) {
    const { from: from25, memo, to: to2, token } = parameters;
    const { address, decimals } = resolveToken(client, { token });
    const value = toBaseUnits(parameters.amount, decimals);
    const callArgs = (() => {
      if (memo && from25)
        return {
          functionName: "transferFromWithMemo",
          args: [from25, to2, value, padLeft2(memo, 32)]
        };
      if (memo)
        return {
  ...
  transfer5.estimateGas = estimateGas;
  async function simulate(client, parameters) {
    return simulateContract(client, {
      ...pickWriteParameters(parameters),
      ...transfer5.call(client, parameters)
...
(function(unpause3) {
  async function inner(action, client, parameters) {
    const { token, ...rest } = parameters;
    const call2 = unpause3.call(client, { token });
    return await action(client, {
      ...rest,
      ...call2
    });
...
(function(prepareUpdateQuoteToken3) {
  async function inner(action, client, parameters) {
    ...
    const call2 = prepareUpdateQuoteToken3.call(client, { quoteToken, token });
    return await action(client, {
      ...rest,
      ...call2
    });
...
(function(updateQuoteToken3) {
  async function inner(action, client, parameters) {
    const { token, ...rest } = parameters;
    const call2 = updateQuoteToken3.call(client, { token });
    return await action(client, {
      ...rest,
      ...call2
    });
...
(function(validate13) {
  ...
  validate13.call = call;
})(validate12 || (validate12 = {}));
...
async function encryptDepositPayload(publicKey, recipient, portalAddress, keyIndex, memo = zeroHash) {
  ...
  const nonce3 = random(12);
  const plaintext = buildDepositPlaintext(recipient, memo);
  const ciphertextWithTag = new Uint8Array(await globalThis.crypto.subtle.encrypt({ name: "AES-GCM", iv: nonce3, tagLength: 128 }, aesKey, from(plaintext)));
...
// node_modules/viem/_esm/tokens/defineToken.js
...
// node_modules/viem/_esm/tokens/definitions/alphausd.js
var alphausd = defineToken({
  addresses: {
...
// node_modules/viem/_esm/tokens/definitions/betausd.js
var betausd = defineToken({
  addresses: {
    42431: "0x20c0000000000000000000000000000000000002"
    // tempoModerato
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/brla.js
var brla = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000f047dd7018e50367"
    // tempo
  },
  currency: "BRL",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/cbbtc.js
var cbbtc = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000c412ec89d0c08be5"
    // tempo
  },
  currency: "BTC",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/chfau.js
var chfau = defineToken({
  addresses: {
    4217: "0x20c00000000000000000000042109aef2f8b28e1"
    // tempo
  },
  currency: "CHF",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/cirbtc.js
var cirbtc = defineToken({
  addresses: {
    1: "0x72DFB2E44f59C5AD2bAFE84314E5b99a7cd5075E",
    // mainnet
    5042002: "0xf0C4a4CE82A5746AbAAd9425360Ab04fbBA432BF",
    // arcTestnet
    11155111: "0x3a3fe695F684Bf9b9e43CF43C2b895Ea5e392bB3"
    // sepolia
  },
  currency: "BTC",
  decimals: 8,
...
// node_modules/viem/_esm/tokens/definitions/cusd.js
var cusd = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000000520792dcccccccc"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/dlusd.js
var dlusd = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000006fd9a167923ba194"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/eurau.js
var eurau = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000009a4a4b17e0dc6651"
    // tempo
  },
  currency: "EUR",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/eurc.js
var eurc = defineToken({
  addresses: {
    1: "0x1aBaEA1f7C830bD89Acc67eC4af516284b1bC33c",
    // mainnet
    25: "0xA6dE01a2d62C6B5f3525d768f34d276652C554c8",
    // cronos
    338: "0x31f7538adb53cF16350e6B0c89d03D91b7D12c46",
    // cronosTestnet
    480: "0x1C60ba0A0eD1019e8Eb035E6daF4155A5cE2380B",
    // worldchain
    4801: "0xe479EcA5740Ac65d6E1823bea2f1C08Bc14e954F",
    // worldchainSepolia
...
// node_modules/viem/_esm/tokens/definitions/eurce.js
var eurce = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000001621e21f71cf12fb",
    // tempo
    42431: "0x20c000000000000000000000d72572838bbee59c"
    // tempoModerato
  },
  currency: "EUR",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/frxusd.js
var frxusd = defineToken({
  addresses: {
...
// node_modules/viem/_esm/tokens/definitions/gbpa.js
var gbpa = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000000a6da882d075a4c3"
    // tempo
  },
  currency: "GBP",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/gusd.js
var gusd = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000005c0bac7cef389a11"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/iusd.js
var iusd = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000ab02d39df30bd17e"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/pathusd.js
var pathusd = defineToken({
  addresses: {
...
// node_modules/viem/_esm/tokens/definitions/reusd.js
var reusd = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000383a23bacb546ab9"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/rusd.js
var rusd = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000007f7ba549dd0251b9"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/sbc.js
var sbc = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000ae247a1130450f09"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/siusd.js
var siusd = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000048c8f36df1c9a4a"
    // tempo
  },
  currency: "siUSD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/stcusd.js
var stcusd = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000008ee4fcff88888888"
    // tempo
  },
  currency: "stcUSD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/susde.js
var susde = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000bd95bfb69fbe6ce3"
    // tempo
  },
  currency: "sUSDe",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/syrupusdc.js
var syrupusdc = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000008191667423f70e67"
    // tempo
  },
  currency: "syrupUSDC",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/thetausd.js
var thetausd = defineToken({
  addresses: {
    42431: "0x20c0000000000000000000000000000000000003"
    // tempoModerato
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/usd1.js
var usd1 = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000111111111e910f0f"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/usdb.js
var usdb = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000003158081efd85bfc2"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/usdc.js
var usdc = defineToken({
  addresses: {
...
// node_modules/viem/_esm/tokens/definitions/usdce.js
var usdce = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000b9537d11c60e8b50",
    // tempo
    42431: "0x20c0000000000000000000009e8d7eb59b783726"
    // tempoModerato
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/usde.js
var usde = defineToken({
  addresses: {
    4217: "0x20c0000000000000000000002f52d5cc21a3207b"
    // tempo
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/usdt0.js
var usdt0 = defineToken({
  addresses: {
...
// node_modules/viem/_esm/tokens/definitions/usyc.js
var usyc = defineToken({
  addresses: {
    1: "0x136471a34f6ef19fE571EFFC1CA711fdb8E49f2b",
    // mainnet
    56: "0x8D0fA28f221eB5735BC71d3a0Da67EE5bC821311",
    // bsc
    97: "0x109656Aba6F175c634c63C9874f29CeAAAB8E606",
    // bscTestnet
    5042002: "0xe9185F0c5F296Ed1797AaE4238D26CCaBEadb86C",
    // arcTestnet
    11155111: "0x38D3A3f8717F4DB1CcB4Ad7D8C755919440848A3"
    // sepolia
  },
  currency: "USD",
  decimals: 6,
...
// node_modules/viem/_esm/tokens/definitions/wsrusd.js
var wsrusd = defineToken({
  addresses: {
    4217: "0x20c000000000000000000000aeed2ec36a54d0e5"
    // tempo
  },
  currency: "wsrUSD",
  decimals: 6,
...
// node_modules/viem/_esm/tempo/Scopes.js
var Scopes_exports = {};
...
// node_modules/viem/_esm/tempo/Selectors.js
var Selectors_exports = {};
__export(Selectors_exports, {
  accountKeychain: () => accountKeychain,
  addressRegistry: () => addressRegistry3,
  feeAmm: () => feeAmm2,
  feeManager: () => feeManager3,
  nonce: () => nonce2,
  receivePolicyGuard: () => receivePolicyGuard3,
  signatureVerifier: () => signatureVerifier,
  stablecoinDex: () => stablecoinDex3,
  tip20: () => tip202,
  tip20ChannelReserve: () => tip20ChannelReserve2,
  tip20Factory: () => tip20Factory3,
  tip403Registry: () => tip403Registry3,
  validatorConfig: () => validatorConfig2,
...
function target(address) {
  return {
    ...
    selector: (selector, options) => ({
      address,
      ...options && "recipients" in options ? { recipients: options.recipients } : {},
      selector
    })
  };
}
...
function tip203(address) {
  return contract(address, tip202);
}
    ...
    return {
      ...
      async request(args) {
        ...
        if (request.method === "wallet_sendCalls") {
          ...
          const { capabilities, chainId, from: from25 } = params;
          const { sync, ...properties } = capabilities ?? {};
          if (!chainId)
            throw new UnsupportedChainIdError();
          if (Number(chainId) !== client.chain.id)
            throw new UnsupportedChainIdError();
          if (from25 && !isEqual2(from25, account.address))
...
function getMetadata2(config, parameters) {
  const { chainId, ...rest } = parameters;
  const client = config.getClient({ chainId });
...
(function(getMetadata3) {
  ...
  function queryOptions(config, parameters) {
    ...
    return {
      ...
      async queryFn(context) {
        const [, parameters2] = context.queryKey;
        return await getMetadata3(config, parameters2);
      }
    };
  }
...
async function encryptDepositPayload2(publicKey, recipient, memo = zeroHash) {
  ...
  const nonce3 = Bytes_exports.random(12);
  const plaintext = encodeAbiParameters([{ type: "address" }, { type: "bytes32" }], [recipient, memo]);
  const ciphertextWithTag = new Uint8Array(await globalThis.crypto.subtle.encrypt({ name: "AES-GCM", iv: nonce3, tagLength: 128 }, aesKey, Bytes_exports.from(plaintext)));
  const ciphertext = ciphertextWithTag.slice(0, -16);
  const tag = ciphertextWithTag.slice(-16);
