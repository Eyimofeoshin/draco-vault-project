...
async function resolveNode() {
  ...
  const { wasmBase64 } = await import("./mine.wasm-VUWYUOZV.js");
...
function resolveToken(client, parameters2) {
  const { decimals, token } = parameters2;
  ...
  return {
    address: address2,
    decimals: decimals ?? inferDecimals(client, address2)
  };
}
function findDeclaredToken(client, token) {
  var _a2;
  const tokens = client.tokens;
  const chainId = (_a2 = client.chain) == null ? void 0 : _a2.id;
...
function findDeclaredTokenBySymbol(client, symbol) {
  ...
  for (const token of tokens) {
    if (((_b = token.symbol) == null ? void 0 : _b.toLowerCase()) === lowerSymbol)
...
async function resolveTokenWithDecimals(client, parameters2) {
  const { address: address2, decimals } = resolveToken(client, parameters2);
  if (decimals !== void 0)
...
function pickWriteParameters(parameters2) {
  const { account, chain, feePayer, feeToken, gas, keyAuthorization, maxFeePerGas, maxPriorityFeePerGas, nonce: nonce2, nonceKey, validAfter, validBefore } = parameters2;
  return {
    account,
    chain,
    feePayer,
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-M7EHKWBG.js.map
